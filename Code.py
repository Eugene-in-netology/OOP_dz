class OpenFiles:
  ...
  def open_files(self):
    if self.file_name_list is None:
      print('Список файлов не передан')
    elif len(self.file_name_list) <= 0:
      return print('Список файлов пуст')
    else:
      ...
      if type(self.our_product_list) is list and len(self.our_product_list) > 0:
        for product in self.our_product_list:
            if product in self.build_cook_book_dict():
                ...
                print(f'{product} не найден в списке блюд')
      else:
        if type(self.our_product_list) is not list:
          return 'Список блюд не обнаружен'
        else:
          return 'Список пуст'
  ...
  def get_ingridients_list(self, dishes, person_count):
    if len(self.our_cook_book.keys()) > 0:
        if len(self.dishes) > 0:
          ...
          print('Блюда из списка для подбора игридиентов не указаны')
    else:
      print('Список блюд пуст или не указан')
...
print()
print(GetCookBook(['recipes.txt'], ['Сникерс', 'шоколад "Алёнка"', 'сосиски деликатесные "Красная цена"', 'Фахитос']).get_cook_book_dict())
print()
...
print()
get_shop_list_by_dishes(['Сникерс', 'шоколад "Алёнка"', 'сосиски деликатесные "Красная цена"', 'Запеченный картофель', 'Омлет'], 2)
print()
